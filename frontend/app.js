...
const state = {data:null, view:"dashboard", page:1, query:"", source:"", sentiment:"", busy:false, report:"Executive Reputation Summary", analyticsTab:"overview", config:null};
...
const nav = {dashboard:"Dashboard",reviews:"Reviews",analytics:"Analytics",insights:"AI Insights",settings:"Settings"};
const paths = {
...
function dashboard(){
 ...
 const score=m.health_breakdown||{};
 const ratedReviews=Number(score.rated_reviews||0), socialComments=Number(score.social_comments||0);
 const ratedWeight=ratedReviews.toFixed(2), socialWeight=(socialComments*.05).toFixed(2), totalWeight=Number(score.weighted_records||0).toFixed(2);
 const scoreFormula='<b>Current calculation</b><br>'+ratedReviews+' rated reviews × 100% = <b>'+ratedWeight+'</b> effective weight<br>'+socialComments+' social comments × 5% = <b>'+socialWeight+'</b> effective weight<br><span class="formula-note">Total effective weight: '+totalWeight+'</span><br><br>Negative signals: −'+(score.negative_penalty??0)+' pts<br>Critical signals: −'+(score.critical_penalty??0)+' pts<br>Average risk: −'+(score.risk_penalty??0)+' pts<br><br><b>Result: '+(m.health??"—")+' / 100</b>';
...
function analytics(){
 const tabs=[["overview","Overview"],["sources","Sources"],["reports","Reports"]];
 const selector='<div class="sentiment-tabs analytics-tabs">'+tabs.map(([key,label])=>'<button data-analytics-tab="'+key+'" class="'+(state.analyticsTab===key?"active":"")+'">'+label+'</button>').join("")+'</div>';
 const overview='<div class="grid two">'+card("Sentiment over time",timeline())+card("Feedback by source",bars(state.data.sources.filter(s=>s.count)))+card("Top complaint topics",bars(state.data.issues,"#D64545"))+card("Positive themes",bars(state.data.positive_themes,"#20A464"))+card("Review volume",volumeChart(),"Monthly count of all dated reviews, including pending analysis")+'</div>';
 const content=state.analyticsTab==="sources"?sourceContent():state.analyticsTab==="reports"?reportContent():overview;
 return head("Analytics","See where feedback comes from, manage sources, and prepare reports.")+selector+content;
}
function insights(){
...
function render(){
 ...
 $("#page").innerHTML=({dashboard,reviews,analytics,insights,settings}[state.view])();
}
function go(view){
 if($("#detail").open)$("#detail").close();
 if(view==="sources"||view==="reports"){state.analyticsTab=view;view="analytics";}
...
function showSyncProgress(status){
 const meter=$("#sync-meter"),label=$("#sync-label"),button=$("#sync-sources");
 const failedSources=Object.entries(status.sources||{}).filter(([,item])=>item.error).map(([source])=>source);
 if(meter){meter.hidden=false;meter.max=status.total||5;meter.value=status.completed||0;}
 if(label){label.textContent=status.running?"Checking "+(status.current||"sources")+" · "+(status.completed||0)+" of "+(status.total||5):failedSources.length?"Finished with an Apify access error: "+failedSources.join(", ")+".":status.current==="Complete"?status.saved+" new reviews saved · "+status.analyzed+" analyzed.":status.current||"Source check stopped.";}
 if(button){button.disabled=Boolean(status.running);button.textContent=status.running?"Checking…":"Check new feedback";}
}
async function waitForSync(){
 const status=await api("/api/sync/status");showSyncProgress(status);
 if(status.running){setTimeout(()=>waitForSync().catch(e=>toast(e.message)),1500);return;}
 await load();
 const issues=Object.entries(status.sources||{}).filter(([,value])=>value.error).map(([source])=>source);
 toast(status.saved+" new reviews saved; "+status.analyzed+" analyzed."+(issues.length?" Check: "+issues.join(", ")+".":""));
 writing=false;$("#refresh").disabled=false;
}
async function syncSources(){
 if(writing||state.busy)return;writing=true;$("#refresh").disabled=true;
 try{
  showSyncProgress({running:true,completed:0,total:5,current:"Starting source check"});
  await api("/api/sync",{method:"POST"});
  waitForSync().catch(e=>{writing=false;$("#refresh").disabled=false;toast(e.message)});
 }catch(e){writing=false;$("#refresh").disabled=false;toast(e.message)}
}
document.addEventListener("click",e=>{
 const b=e.target.closest("button");if(!b)return;
 if(b.dataset.view)go(b.dataset.view);
 if(b.dataset.go)go(b.dataset.go);
 if(b.dataset.analyticsTab){state.analyticsTab=b.dataset.analyticsTab;state.view="analytics";location.hash="analytics";render();window.scrollTo(0,0);}
 if(b.dataset.analyticsTab){return;}
 ...
 if(b.dataset.analyticsTab){state.analyticsTab=b.dataset.analyticsTab;render();}
 ...
 if(b.id==="score-info"){const formula=$("#score-formula"),open=formula.hidden;formula.hidden=!open;b.setAttribute("aria-expanded",String(open));}
 if(b.id==="sync-sources")syncSources();
});
...
$("#nav").innerHTML=Object.entries(nav).map(([k,v])=>'<button data-view="'+k+'"><span>'+v+'</span></button>').join("");
$("#refresh").innerHTML=icon("refresh");$("#menu").innerHTML=icon("menu");
$("#refresh").onclick=syncSources;
